...
SampleSizeSchizo = length(unique(data2$participant[data2$diagnosis == "schizophrenia"]))
SampleSizeContros = length(unique(data2$participant[data2$diagnosis == "control"]))

PitchMeanControls = mean(data2$mean[data2$diagnosis == "control"])
PitchMeanSchizo = mean(data2$mean[data2$diagnosis == "schizophrenia"])
PitchMeanControlsSD = sd(data2$mean[data2$diagnosis == "control"])
PitchMeanSchizoSD = sd(data2$mean[data2$diagnosis == "schizophrenia"])

PitchSDControls = mean(data2$stdDev[data2$diagnosis == "control"])
PitchSDSchizo = mean(data2$stdDev[data2$diagnosis == "schizophrenia"])
PitchSDControlsSD = sd(data2$stdDev[data2$diagnosis == "control"])
PitchSDSchizoSD = sd(data2$stdDev[data2$diagnosis == "schizophrenia"])
Article = "Assignment3"
Year = 2017
